...
from requests import get, HTTPError, Session
...
http_adapter.max_retries.respect_retry_after_header = False
...
def main(titleid, titlever=None, pack_as_wad=True, decryptcontents=False, localuse=True, keepcontents=True,
    ...
    for i, content_url in enumerate(nus.get_content_urls()):
        ...
        with http_session.get(content_url, timeout=30, stream=True) as req:
            ...
            for chunk in req.iter_content(chunk_size=(1024 * 256)):
                ...
                if progress < content_total_length:
                    sys.stdout.write("\r      %u%% [%s%s] %s / %s" % (percentage, '=' * bar_length, ' ' * (50 - bar_length), utils.convert_size(progress), content_total_length_str))
                else:
                    sys.stdout.write("\r      100%% [%s] %s / %s" % ('=' * 50, content_total_length_str, content_total_length_str))
                sys.stdout.flush()
        ...
        if cetk:
            valid, decdata = utils.Crypto.check_content_hash(tmd.contents[i], cetk, content_data, return_decdata=True)
            if not valid:
